...
def get_dataset(p):
    (
        ...
        dst_train,
        dst_test,
    ) = datasets.__dict__[p.dataset](p.dataset_dir)
    p.channel, p.im_size, p.num_classes, p.class_names, p.mean, p.std = (
        channel,
        im_size,
        num_classes,
        class_names,
...
def get_model(p, device):
    model = nets.__dict__[p.model](
        p.channel, p.num_classes, im_size=p.im_size, pretrained=p.pretrained
    ).to(device)
    if device == "cpu":
        print("Using CPU.")
...
def get_scheduler(p, optimizer):
    if p.scheduler == "reduceonplateau":
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, factor=p.factor, patience=p.patience, verbose=True
        )
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(
            ...
            max_lr=p.max_lr,
            epochs=p.epochs,
            steps_per_epoch=p.len_loader,
            div_factor=p.max_lr / p.lr,
            final_div_factor=p.lr / p.min_lr,
            verbose=False,
        )
    elif p.scheduler == "exponentiallr":
        scheduler = optim.lr_scheduler.ExponentialLR(
            optimizer, gamma=p.gamma, verbose=True
        )
    ...
    # add  elif for cosine annealing lr
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=p.T_max, eta_min=p.min_lr, verbose=False
        )
    elif p.scheduler == "cosineannealingwarmrestarts":
        scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=p.T_max, T_mult=p.T_mult, eta_min=p.min_lr, verbose=False
        )
    else:
...
def get_parser():
    ...
    parser.add_argument(
        "--pretrained", type=str_to_bool, default=False, help="pretrained"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of iterations for each coreset",
    )
    parser.add_argument(
        "--batch_size", "-bs", type=int, default=1000, help="Batch size"
    )
    parser.add_argument(
        "--val_batch_size", "-vbs", type=int, default=1000, help="Validation batch size"
    )
    ...
    parser.add_argument(
        "--min_lr", type=float, default=0.0001, help="min learning rate"
    )
    ...
    parser.add_argument(
        "--scheduler", type=str, default="reduceonplateau", help="scheduler"
    )
    parser.add_argument(
        "--patience", type=int, default=50, help="Patienece for ReduceOnPlateau"
    )
    parser.add_argument(
        "--factor", type=float, default=0.2, help="Factor for ReduceOnPlateau"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--T_max", type=int, default=50, help="T_max for CosineAnnealingLR"
    )
    parser.add_argument(
        "--T_mult", type=int, default=1, help="T_mult for CosineAnnealingLR"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Early Stopping Patience (default: -1 i.e no early stopping)",
    )
    parser.add_argument(
        "-esd",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Minimum number of epochs for early stopping (default: 100)",
    )
    # Misc parameters
    ...
    parser.add_argument(
        ...
        type=str_to_bool,
        default=False,
        help="Specify if per class mean gradients or not",
    )
    parser.add_argument(
        "--class_balanced",
        type=str_to_bool,
        default=False,
        help="whether to use class balanced sampling",
    )
    parser.add_argument(
        "--with_train", type=str_to_bool, default=False, help="With train"
    )
    parser.add_argument(
        "--random",
        type=str_to_bool,
        default=False,
        help="whether to use random sampling for coreset",
    )
    parser.add_argument("--augment", type=str_to_bool, default=False, help="Augment")
    parser.add_argument(
        "--temp", type=str_to_bool, default=False, help="whether to use temp folder"
    )
    # parameters for train.py
    parser.add_argument(
        "--train", type=str_to_bool, default=True, help="whether to train"
    )
    parser.add_argument(
        "--test_model", type=str, default=None, help="path to test model"
    )
